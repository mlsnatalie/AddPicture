...
public class DisplayUtil {
    ...
    public static boolean hasNavBar(Activity activity) {
        int virtualHeight = getVirtualBarHeight(activity);
        if (virtualHeight <= 0) {
            return false;
        }
        int navHeight = virtualHeight;
        View view = activity.findViewById(android.R.id.content);
        if (view == null) {
            return true;
        }
        int rootHeight = view.getBottom();
        int screenHardwareHeight = getRealMetricsHeigh(activity);
        return navHeight != 0 && screenHardwareHeight - rootHeight > virtualHeight / 2;
    }
    ...
    public static int getRealMetricsHeigh(Context context) {
        WindowManager windowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        Display display = windowManager.getDefaultDisplay();
        DisplayMetrics dm = new DisplayMetrics();
        display.getRealMetrics(dm);
